"""
╔══════════════════════════════════════════════════════════════════════╗
║   FLEET RUNNING HOURS MONITORING SYSTEM  v6.0                        ║
║   Wide-Row Parser · 100% Data Integrity · Enterprise Cloud Edition   ║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
.streamlit-expanderHeader { background: var(--bg3) !important; border: 1px solid var(--b2) !important; border-radius: 8px !important; font-family: var(--ff) !important; font-weight: 500 !important; font-size: .85rem !important; color: var(--t1) !important; transition: background .2s, border-color .2s !important; }
...
.vc-stat-lbl { font-family: var(--fi); font-size: .58rem; text-transform: uppercase; letter-spacing: .14em; color: var(--t3); margin-top: 2px; }
.sev-badge { border-radius: 6px; padding: 5px 13px; font-family: var(--fm); font-size: .68rem; font-weight: 700; letter-spacing: .07em; }
...
def _cp(raw):
    ...
    s = re.sub(r'\.(?=\d{3}(\.|$))','',str(raw).strip())
    s = re.sub(r'[^0-9\.]','',s)
    try: return float(s) if s else None
    except ValueError: return None

def _pd(raw):
    if not raw or str(raw).strip() in ('','N/A','n/a','-','--'): return None
    raw = re.sub(r'\s+',' ',str(raw).strip().lstrip('[').rstrip(']'))
    if re.match(r'^\d+$',raw.strip()): return None # Ignore pure integers
...
def _ph(raw):
    if not raw or str(raw).strip() in ('','N/A','n/a','-','--'): return None
    for n in re.findall(r'\d[\d,]*',str(raw).replace('\n',' ')):
        try:
            v=float(n.replace(',',''))
            if v>0: return v
        except ValueError: pass
...
def parse_wide_matrix(table, category: str, start_seq: int) -> tuple[list, int]:
    ...
    grid = [[str(c.text).strip() for c in r.cells] for r in table.rows]
    
    # Map Columns by dynamic header search
    cyl_map = []
    for r in grid[:7]: 
...
def parse_doc_bytes(docx: bytes) -> dict:
    ...
    seq = 0
    if len(doc.tables) > 0:
        me_comps, seq = parse_wide_matrix(doc.tables[0], 'MAIN_ENGINE', seq)
        comps.extend(me_comps)
    else: warns.append("Missing Main Engine matrix.")
        
    if len(doc.tables) > 2:
        aux_comps, seq = parse_wide_matrix(doc.tables[2], 'AUX_ENGINE', seq)
...
def save_parsed(parsed, filename, fhash):
    ...
    try:
        ...
        # Deletes existing components ONLY for this specific vessel to prevent database bloat, then injects the new wide-matrix lines
...
with st.sidebar:
    ...
    st.markdown(f'<div style="font-family:var(--fm);font-size:.58rem;color:var(--t3)">'
                f'db {db_kb:.0f} kb · {len(vessels)} vessels · v6.0</div>',
                unsafe_allow_html=True)

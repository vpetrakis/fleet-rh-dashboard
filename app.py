...
def _matrix_html(records: List[Dict], mode: str = 'matrix') -> str:
    ...
    for rec in _sort_recs(records, mode):
        ...
        body += (
            ...
            + td(rec.get('description', ''), c['comp'], '600', 'left', "'IBM Plex Sans',sans-serif", '.80rem', '320px')
...
with st.expander("Upload TEC-004 Report", expanded=(st.session_state.data is None)):
    ...
    if uploaded:
        ...
        if st.session_state.data is None or st.session_state.data.get('_hash') != fh:
            with st.spinner('Converting .doc to .docx via LibreOffice...'):
                try:
...
v = me[:]
if mc_sel != 'All':
    v = [c for c in v if c['description'] == mc_sel]
if ms_sel == 'Overdue only':
    v = [c for c in v if c['status'] == 'OVERDUE']
elif ms_sel == 'High Priority +':
    v = [c for c in v if c['status'] in ('OVERDUE', 'HIGH PRIORITY')]
elif ms_sel == 'OK only':
    v = [c for c in v if c['status'] == 'OK']
...
v = aux[:]
if ae_sel != 'All':
    v = [c for c in v if c['engine_label'] == ae_sel]
if ac_sel != 'All':
    v = [c for c in v if c['description'] == ac_sel]
if as_sel == 'Overdue only':
    v = [c for c in v if c['status'] == 'OVERDUE']
elif as_sel == 'High Priority +':
    v = [c for c in v if c['status'] in ('OVERDUE', 'HIGH PRIORITY')]
elif as_sel == 'OK only':
    v = [c for c in v if c['status'] == 'OK']
st.markdown(_rec_line(v), unsafe_allow_html=True)

...
def _matrix_html(records: List[Dict], mode: str='matrix') -> str:
    ...
    header='<tr style="background:#020508">' + ''.join(f'<th style="{TH}">{h}</th>' for h in HEADS) + '</tr>'
    body=''
    for rec in _sort_recs(records,mode):
        ...
        body+=(
            f'<tr style="background:{c["row"]};border-bottom:1px solid {c["bord"]};transition:filter .1s" '

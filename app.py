...
:root {
  --bg: #071019; --bg2: #0c1623; --bg3:#102033; --line: #1b2d44; --gold: #c99818;
  --t0: #ebf3ff; --t1: #a9bdd4; --ok:#1f9d55; --warn:#c27a00; --bad:#c0392b; --info:#2d7ff9;
}
...
.badge-ok,.badge-warn,.badge-bad,.badge-info { display:inline-block; padding:.25rem .55rem; border-radius:999px; font-size:.75rem; font-weight:600; margin-right:.4rem; }
...
def convert_doc_to_docx(raw: bytes) -> bytes:
    ...
    if not os.path.isfile(soffice):
        raise RuntimeError("LibreOffice not found in runtime environment.")
    ...
    try:
        proc = subprocess.run(
            [soffice, "--headless", "--norestore", "--nofirststartwizard", f"-env:UserInstallation={profile}", "--convert-to", "docx", src, "--outdir", outdir],
            capture_output=True,
            timeout=120,
            text=True,
        )
...
def extract_me(table_rows: List[List[str]], warnings: List[WarningItem]) -> List[Dict[str, Any]]:
    ...
    for row in table_rows:
        ...
        if any("CYL. NO." in c for c in normed):
            cyl_headers = [c for c in row if "CYL." in c.upper()]
        comp = normalize_token(row[0]) if row else ""
        if comp in ME_COMPONENTS and len(row) >= 4:
            period = parse_num(row[1])
            if period is None and "OBSERVATION" not in normalize_token(row[1] if len(row) > 1 else ""):
                warnings.append(WarningItem("Main Engine", "warning", f"Missing periodicity for {comp}", " | ".join(row)))
            if not re.fullmatch(r"1", normalize_token(row[2])):
                continue
            if len(records) >= 0:
                pass
    i = 0
    while i < len(table_rows) - 1:
        ...
        if comp in ME_COMPONENTS and len(row1) >= 4 and len(row2) >= 4 and normalize_token(row2[2] if len(row2) > 2 else "") == "2":
            ...
            for j in range(cyl_count):
                ...
                if bad_date:
                    warnings.append(WarningItem("Main Engine", "warning", f"Invalid date for {comp} cyl {j+1}: {bad_date}", raw_date))
...
def extract_aux(table_rows: List[List[str]], warnings: List[WarningItem]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    while i < len(table_rows) - 1:
        ...
        if comp in AUX_COMPONENTS:
            ...
            if bad_date:
                warnings.append(WarningItem("Aux Engine", "warning", f"Invalid date for {comp}: {bad_date}", date_cell))
            records.append({
                "Status": get_status(hrs, period),
                "Component": comp,
                "Engine": "AUX-1",
    ...
    for row in table_rows:
        ...
        for idx, cell in enumerate(cells):
            ...
            if comp in OE_COMPONENTS:
                ...
                hrs = parse_num(raw_hrs)
                if bad_date:
                    warnings.append(WarningItem("Other Equipment", "warning", f"Invalid date for {comp}: {bad_date}", raw_date))
...
if uploaded:
    with st.spinner("Parsing TEC-004 with validation and confidence scoring..."):
        try:
            ...
            st.markdown(f'<span class="badge-bad">Overdue: {n_od}</span><span class="badge-warn">High priority: {n_hp}</span><span class="badge-info">Warnings: {len(warnings)}</span>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="panel"><div class="small">Upload a TEC-004 monthly running-hours report in .doc or .docx format. This build prioritizes structural parsing, validation, and review visibility over silent best-effort extraction.</div></div>', unsafe_allow_html=True)

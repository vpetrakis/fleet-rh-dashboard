"""
╔══════════════════════════════════════════════════════════════════════╗
║      FLEET RUNNING HOURS MONITORING SYSTEM  v2.0                    ║
...
import os, re, sqlite3, io, tempfile, hashlib, subprocess
from datetime import datetime
from pathlib import Path
import pandas as pd

# ════════════════════════════════════════════════════════════════════
# SECTION 1 — PREMIUM CSS
# ════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Barlow:ital,wght@0,300;0,400;0,500;0,600;0,700;1,300&family=Barlow+Condensed:wght@300;400;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap');
...
.main::before {
  ...
  background:
    radial-gradient(ellipse 60% 40% at 15% 20%, rgba(59,130,246,0.04) 0%, transparent 60%),
...
[data-testid="stMetric"] {
  background: linear-gradient(135deg, var(--panel) 0%, var(--navy3) 100%) !important;
  border: 1px solid var(--border) !important;
...
.stButton > button {
  ...
  font-family: var(--cond) !important; font-weight: 800 !important; font-size: 0.88rem !important;
  letter-spacing: 0.1em !important; text-transform: uppercase !important;
  border-radius: 6px !important; padding: 0.55rem 1.75rem !important;
  box-shadow: 0 2px 12px rgba(245,158,11,0.25) !important;
  transition: all 0.2s ease !important;
}
.stButton > button:hover {
  background: linear-gradient(135deg, var(--amber2) 0%, var(--amber) 100%) !important;
  box-shadow: 0 4px 20px rgba(245,158,11,0.4) !important; transform: translateY(-2px) !important;
}
.stButton > button:active { transform: translateY(0) !important; }

[data-testid="stFileUploadDropzone"] {
  background: linear-gradient(135deg, rgba(245,158,11,0.04) 0%, rgba(59,130,246,0.04) 100%) !important;
  border: 2px dashed var(--amber) !important; border-radius: 12px !important;
  transition: all 0.3s ease !important; padding: 2rem !important;
}
[data-testid="stFileUploadDropzone"]:hover {
  background: rgba(245,158,11,0.08) !important; border-color: var(--amber2) !important;
  box-shadow: 0 0 30px rgba(245,158,11,0.1) !important;
}
[data-testid="stFileUploadDropzone"] p,
[data-testid="stFileUploadDropzone"] span { color: var(--amber) !important; font-family: var(--cond) !important; font-size: 1rem !important; font-weight: 600 !important; }
[data-testid="stFileUploadDropzone"] small { color: var(--text2) !important; }

.stTabs [data-baseweb="tab-list"] {
  background: var(--panel) !important; border-radius: 10px 10px 0 0 !important;
  border-bottom: 1px solid var(--border) !important; gap: 0 !important; padding: 0 0.5rem !important;
}
.stTabs [data-baseweb="tab"] {
  background: transparent !important; color: var(--muted) !important;
  font-family: var(--cond) !important; font-weight: 600 !important;
  letter-spacing: 0.07em !important; text-transform: uppercase !important;
  font-size: 0.82rem !important; padding: 0.85rem 1.5rem !important;
  border-bottom: 2px solid transparent !important; margin-bottom: -1px !important; transition: color 0.2s !important;
}
...
.streamlit-expanderHeader {
  ...
  border-radius: 8px !important; font-family: var(--cond) !important; font-weight: 600 !important;
  font-size: 0.9rem !important; color: var(--text) !important; letter-spacing: 0.04em !important;
...
.stAlert { border-radius: 8px !important; border-left-width: 3px !important; font-family: var(--font) !important; }
...
.page-header { animation: fadeSlideDown 0.5s ease both; }
.page-sub    { animation: fadeSlideDown 0.5s 0.1s ease both; opacity:0; animation-fill-mode:forwards; }

.kpi-card {
  background: linear-gradient(135deg, var(--panel) 0%, var(--navy3) 100%);
  border: 1px solid var(--border); border-radius: 12px; padding: 1.1rem 1.4rem;
  position: relative; overflow: hidden;
  animation: fadeSlideUp 0.4s ease both; transition: transform 0.2s, border-color 0.2s;
}
...
.section-label {
  font-family:var(--cond); font-size:0.65rem; font-weight:700; letter-spacing:0.18em;
  text-transform:uppercase; color:var(--muted); border-bottom:1px solid var(--border);
  padding-bottom:0.5rem; margin:1.75rem 0 1.1rem;
}

.info-card {
  background: linear-gradient(135deg, var(--panel) 0%, var(--navy3) 100%);
  border: 1px solid var(--border); border-radius: 12px;
  padding: 1.25rem 1.5rem; font-size: 0.82rem; color: var(--text2); line-height: 1.8;
}
.info-card-title {
  font-family:var(--cond); font-size:0.72rem; font-weight:700; letter-spacing:0.12em;
...
.parse-stat {
  ...
  padding:0.6rem 1rem; min-width:100px; animation:fadeSlideUp 0.4s ease both;
}
...
.success-banner {
  ...
  padding: 1rem 1.5rem; color: #6ee7b7; font-family: var(--cond);
...
.conv-badge {
  ...
  border-radius:20px; padding:3px 10px; font-size:0.7rem; color:#93c5fd;
  font-family:var(--cond); font-weight:600; letter-spacing:0.06em;
}
...
# ════════════════════════════════════════════════════════════════════
# SECTION 3 — PARSER  (100% integrity validated)
# ════════════════════════════════════════════════════════════════════
...
def convert_doc_to_docx(file_bytes: bytes) -> bytes:
    """Convert .doc → .docx via LibreOffice headless (installed via packages.txt)."""
    ...
    try:
        result = subprocess.run(
            ['soffice', '--headless', '--convert-to', 'docx', tmp_path, '--outdir', out_dir],
        ...
        if result.returncode != 0 or not os.path.exists(docx_path):
            ...
            raise RuntimeError(
                f"LibreOffice conversion failed (code {result.returncode}). {err[:200] or 'No details.'}"
            )
...
def parse_doc(file_bytes: bytes) -> dict:
    ...
    if vessel_name == "UNKNOWN":
        warns.append("Could not extract vessel name from document header.")
    ...
    # Detect cylinder columns (deduplicate merged cells)
    cyl_cols = []
    if len(t0.rows) > 1:
    ...
    while i < len(rows) - 1:
        ...
        t1_type = r1[2] if len(r1) > 2 else ''
        t2_type = r2[2] if len(r2) > 2 else ''
        if t1_type == '1' and t2_type == '2' and r1[0] == (r2[0] if r2 else ''):
            ...
            for ci, lbl in cyl_cols:
                ...
                components.append({'category':'MAIN_ENGINE','engine_label':'ME','unit':lbl,
                    'description':name,'periodicity':period,'last_oh_date':d,'last_oh_hrs':h,
                    'hrs_since':h,'pct_used':_pct(h,period),'status':_status(h,period)})
            i += 2
        else:
            i += 1
    ...
    if len(doc.tables) > 1:
        ...
        skip = {'TURBOCHARGER','AUXILIARY BOILER','COOLERS','EXH GAS  BOILER',
                ...
                'PERIODICTLY','DATE OF LAST INSPECTION','RUN HRS','DATE OF LAST CLEANING','DATE','PERIODICITY'}
        for ridx, row in enumerate(t1.rows):
            cells = [c.text.strip() for c in row.cells]
            for sec_name, dc, datec, hrsc in [
                ...
                if desc and (dv or hv):
                    other_equip.append({'section':sec_name,'description':desc,'periodicity':'','last_date':dv,'run_hrs':hv})
        ...
        if rows2:
            ...
            for ci, cell in enumerate(hdr):
                if m := re.search(r'Aux\.\s*Engine\s*No\.?\s*(\d+)', cell, re.IGNORECASE):
                    ...
                    if lbl not in seen:
                        seen.add(lbl)
                        th = next((_parse_hrs(total[j]) for j in range(ci, min(ci+14, len(total))) if _parse_hrs(total[j])), None)
        ...
        while i2 < len(rows2) - 1:
            ...
            if t1t in ('1','2') and r1[0] == (r2[0] if r2 else ''):
                ...
                for ci, (elbl, cn) in cyl_map.items():
                    ...
                    h = _parse_hrs(r2[ci])  if ci < len(r2) else None
                    if d is None and h is None: continue
                    components.append({'category':'AUX_ENGINE','engine_label':elbl,'unit':f"Cyl {cn}",
                        'description':name,'periodicity':period,'last_oh_date':d,'last_oh_hrs':h,
                        'hrs_since':h,'pct_used':_pct(h,period),'status':_status(h,period)})
                i2 += 2
            else:
    ...
    if len(doc.tables) > 3:
        ...
        for ridx, row in enumerate(t3.rows):
            ...
            for dc, pc, tc, ds in [(0,1,2,3),(9,10,11,12)]:
                desc   = cells[dc] if dc < len(cells) else ''
                period = cells[pc] if pc < len(cells) else ''
                ...
                for dgi, dglbl in enumerate(dglbls):
                    ...
                    if rtype == '1':
                        other_equip.append({'section':'D/G EQUIPMENT','description':key,
                            'periodicity':period,'last_date':_parse_date(val) or val,'run_hrs':''})
                    else:
                        for e in reversed(other_equip):
                            ...
                            other_equip.append({'section':'D/G EQUIPMENT','description':key,
                                'periodicity':period,'last_date':'','run_hrs':val})

    return {'vessel_name':vessel_name,'report_date':report_date,
...
def save_parsed_data(parsed, filename, file_hash):
    ...
    for oe in parsed['other_equipment']:
        c.execute("INSERT INTO other_equipment(vessel_name,section,description,periodicity,last_date,run_hrs,updated_at) VALUES(?,?,?,?,?,?,?)",
                  (v,oe['section'],oe['description'],oe.get('periodicity',''),oe.get('last_date',''),oe.get('run_hrs',''),now))
...
def kpi_html(value, label, color="amber", delay=0):
    return f'<div class="kpi-card {color}" style="animation-delay:{delay}s;"><div class="kpi-value">{value}</div><div class="kpi-label">{label}</div></div>'

def colored_status_df(df):
...
with st.sidebar:
    ...
    selected_vessel = st.selectbox("Select Vessel", vessels, label_visibility="visible") if vessels else None
    if not vessels:
    ...
    st.markdown(f'<p style="font-size:0.65rem;color:#334155;font-family:var(--mono);">db: {db_kb:.1f} kb | v2.0</p>', unsafe_allow_html=True)
...
if page == "📤  Upload Report":
    ...
    st.markdown('<div class="page-sub section-label">Drop a TEC-004 Word document — .doc or .docx accepted</div>', unsafe_allow_html=True)

    col_up, col_info = st.columns([3, 2], gap="large")
    with col_up:
        uploaded = st.file_uploader("file", type=["doc","docx"], label_visibility="collapsed")
    with col_info:
        st.markdown("""
        <div class="info-card">
          <div class="info-card-title">Accepted Format</div>
          TEC-004 Running Hours Monthly Report<br>
          Any vessel &nbsp;·&nbsp; <b>.doc</b> or <b>.docx</b><br><br>
    ...
    if uploaded:
        ...
        if uploaded.name.lower().endswith('.doc'):
            with st.spinner("Converting .doc → .docx via LibreOffice…"):
                try:
                    ...
                    st.markdown('<span class="conv-badge">✓ .doc converted successfully</span>', unsafe_allow_html=True)
                except (RuntimeError, FileNotFoundError, OSError) as e:
                    st.error(
                        "**LibreOffice is not available on this server.**\n\n"
                        "**Quick fix:** Save your file as **.docx** in Word "
        ...
        if parsed['components']:
            with st.expander("Preview parsed data before saving", expanded=False):
                st.dataframe(style_status_df(colored_status_df(pd.DataFrame(parsed['components']))),
...
# ── FLEET OVERVIEW ───────────────────────────────────────────────────
    ...
    summary = get_fleet_summary()
    if summary.empty:
        st.info("No vessel data loaded. Go to **Upload Report** to get started.")
        st.stop()
    ...
    for col,(val,lbl,clr,dly) in zip(cols,[
        ...
        with col: st.markdown(kpi_html(val,lbl,clr,dly), unsafe_allow_html=True)
    ...
    st.dataframe(disp.style.apply(fleet_style,axis=1), use_container_width=True, hide_index=True,
    ...
    for _, row in summary.iterrows():
        ...
        lbl  = f"{icon} **{row['vessel_name']}** — {int(row['overdue'])} overdue · {int(row['high_priority'])} high priority · {int(row['ok'])} OK"
        with st.expander(lbl, expanded=False):
            ...
            if not cc.empty:
                ...
                with t1:
                    ...
                    else: st.dataframe(style_status_df(colored_status_df(od)), use_container_width=True, hide_index=True, height=min(400,38*(len(od)+1)))
                with t2:
                    hp = cc[cc['status']=='HIGH PRIORITY']
                    if hp.empty: st.success("No high-priority items.")
                    else: st.dataframe(style_status_df(colored_status_df(hp)), use_container_width=True, hide_index=True, height=min(400,38*(len(hp)+1)))


# ── VESSEL DETAIL ────────────────────────────────────────────────────
elif page == "🚢  Vessel Detail":
    if not selected_vessel:
        st.info("No vessel selected. Upload a report first, then pick a vessel from the sidebar.")
        st.stop()

    st.markdown(f'<div class="page-header"><h1>🚢 {selected_vessel}</h1></div>', unsafe_allow_html=True)
    st.markdown('<div class="page-sub section-label">Detailed component analysis by engine &amp; system</div>', unsafe_allow_html=True)
    ...
    cols = st.columns(5)
    for col,(val,lbl,clr,dly) in zip(cols,[
        (n_total,"Total","amber",0),(n_od,"Overdue","red",0.05),
        (n_hp,"High Priority","orange",0.10),(n_ok,"OK","green",0.15),(n_nd,"No Data","blue",0.20)]):
        with col: st.markdown(kpi_html(val,lbl,clr,dly), unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        if alerts.empty:
            ...
            st.dataframe(style_status_df(colored_status_df(alerts)), use_container_width=True, hide_index=True, height=min(700,38*(len(alerts)+1)))

    with tabs[1]:
        me = df[df['category']=='MAIN_ENGINE']
        if me.empty: st.info("No Main Engine data.")
        else:
            sel = st.selectbox("Filter component", ['ALL']+sorted(me['description'].unique().tolist()), key="me_f")
            v   = me if sel=='ALL' else me[me['description']==sel]
            st.dataframe(style_status_df(colored_status_df(v)), use_container_width=True, hide_index=True, height=min(700,38*(len(v)+1)))

    with tabs[2]:
        aux = df[df['category']=='AUX_ENGINE']
        if aux.empty: st.info("No Aux Engine data.")
        else:
            sel = st.selectbox("Filter engine", ['ALL']+sorted(aux['engine_label'].unique().tolist()), key="aux_f")
            v   = aux if sel=='ALL' else aux[aux['engine_label']==sel]
            st.dataframe(style_status_df(colored_status_df(v)), use_container_width=True, hide_index=True, height=min(700,38*(len(v)+1)))
    ...
    with tabs[4]:
        ...
        filt = df[df['status'].isin(sf) & df['category'].isin(cf)]
        st.dataframe(style_status_df(colored_status_df(filt)), use_container_width=True, hide_index=True, height=min(800,38*(len(filt)+1)))
